...
function snapshotToPdfExperience(
  ...
  return blocks.slice(0, 8).map((b, i) => {
    ...
    return {
      id: `b-${i}`,
      company: parts[0] || "Career",
      title: parts.slice(1).join(" - ") || b,
      location: "",
      start: "",
      end: "",
      bullets: [b],
...
export function JobDetailSheet({
  ...
  const [screening, setScreening] = useState<ScreeningAnswer[]>([]);
  ...
  useEffect(() => {
    ...
    setScreening(existing?.screeningAnswers ?? []);
    setVariantId(
    ...
    buildResumePdf(
      {
        ...
        phone: "(864) 547-5974",
        location: "Charleston, SC | Remote / Hybrid | Open to Relocation",
  ...
  const runPacket = async () => {
    ...
    try {
      ...
      downloadPdf(
        bytes,
        `${slugify("khristian-kline")}-${slugify(job.company)}-packet-resume.pdf`,
      );
      downloadTextFile(
        `${slugify("khristian-kline")}-${slugify(job.company)}-application-packet.txt`,
        packet.plainText,
      );
      try {
        await navigator.clipboard.writeText(packet.plainText);
      } catch {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <ApplicationQuestions
            ...
            onChange={(rows) => {
              setScreening(rows);
              const a = ensureApplication(job.id);
              if (a) updateApplication(a.id, { screeningAnswers: rows });
            }}
          />
          ...
          <section>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => {
                  const note =
                    linkedIn ?? buildLinkedInNote(job, profile, referralPath);
                  setLinkedIn(note);
                  const a = app ?? ensureApplication(job.id);
